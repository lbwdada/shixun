...
<body>
    ...
    <form action="commodity?method=updateCommodity" method="post" enctype="multipart/form-data">
    	<!-- 隐藏域存放当前数据库中的图片信息和cid-->    	
    	<input  type="hidden" name="cid" value="${commodity.cid }"/>
    	<input  type="hidden" name="pic1" value="${commodity.pic }"/>
	    <ul class="forminfo">
	    	 
	    	 <!-- 商品分类信息 -->
	    	 <li>
	    	 	<label>商品分类</label>
	    	 	<select name="gid" class="dfinput">
	    	 		<c:if test="${ list!=null }">
	    	 			<c:forEach items="${list }" var="category">
	    	 				<option value="${category.gid}" <c:if test="${commodity.gid==category.gid }">selected</c:if> >${category.name }</option>
	    	 			</c:forEach>
	    	 		</c:if>
	    	 	</select>
	    	 </li>
		    ...
		    <li><label>商品小类别</label>
			    <cite>
				   <input name="state" type="radio" value="0" <c:if test="${commodity.state==0 }">checked</c:if> />正常&nbsp;&nbsp;&nbsp;&nbsp;
				    ...
				    <input name="state" type="radio" value="2" <c:if test="${commodity.state==2 }">checked</c:if>/>新品
				    <input name="state" type="radio" value="3" <c:if test="${commodity.state==3 }">checked</c:if>/>为你推荐
				    <input name="state" type="radio" value="4" <c:if test="${commodity.state==4 }">checked</c:if>/>明星单品
			    </cite>
		    </li>
